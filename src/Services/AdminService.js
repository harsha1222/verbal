import axios from'axios'
const TRAIN_SEARCH_ALL_URL ='http://localhost:8081/admin/findall';
const TRAIN_ADD_TRAIN_URL ='http://localhost:8081/admin/add';
const TRAIN_GET_TRAIN_BY_ID_URL ='http://localhost:8081/admin/find/{trainNo}';
const TRAIN_UPDATE_TRAIN_BY_ID_URL ='http://localhost:8081/admin/update/{trainNo}';
const TRAIN_DELETE_TRAIN_BY_ID_URL ='http://localhost:8081/admin/delete/{trainNo}';
const TRAIN_ADMIN_LOGIN_URL='http://localhost:8081/admin/signup'


class AdminService{

getAllTrains(){
    return axios.get(TRAIN_SEARCH_ALL_URL)
}

addTrain(TrainDetails){
    return axios.post(TRAIN_ADD_TRAIN_URL ,TrainDetails)
}

getTrainById(trainNo){
    return axios.get(TRAIN_GET_TRAIN_BY_ID_URL + '/'+ trainNo);
}

upDateTrain(trainNo,TrainDetails){
    return axios.put(TRAIN_UPDATE_TRAIN_BY_ID_URL + '/'+ trainNo ,TrainDetails);
}

deleteTrain(trainNo){
    return axios.delete(TRAIN_DELETE_TRAIN_BY_ID_URL + '/' + trainNo);
}

adminLogin(adminDetails){
    return axios.post(TRAIN_ADMIN_LOGIN_URL , adminDetails );
}

}
export default new AdminService();